...
const { createPlaygroundDeepResearchModeStreamHandler } = await import("../src/chat/deep-research.js");
...
test("deep research stream handler emits an error event on failure", async () => {
    ...
    assert.match(String((persisted[0]?.messages[1]?.content)[0]?.text), /Deep Research failed: Sub-agent worker did not become ready within 5000ms/);
});
...
//# sourceMappingURL=deep-research-stream-handler.test.js.map