 ...
 */
import { createFileSystemDeepResearchStore, } from "@agentrail/deep-research";
...
test("deep research artifact route streams raw artifact bytes", async () => {
    ...
    const hostArtifactPath = join(dataDir, "sandboxes", sessionId, ".deep-research", "artifacts", "chart.svg");
    ...
    const app = new Hono();
    app.route("/api/sessions", deepResearch);
    const response = await app.request(`/api/sessions/${sessionId}/deep-research/artifact?path=${encodeURIComponent("/workspace/.deep-research/artifacts/chart.svg")}`);
    assert.equal(response.status, 200);
    assert.equal(response.headers.get("content-type"), "image/svg+xml");
    assert.match(await response.text(), /<svg/);
});
//# sourceMappingURL=deep-research-route.test.js.map