...
test("worker polls mailbox and drains pending work without an explicit wake", async () => {
    ...
    try {
        ...
        await waitForMessage(messages, (message) => message.type === "job_started" && String(message.jobId).startsWith("job:input-worker-1"));
        await waitForMessage(messages, (message) => message.type === "job_completed" &&
            message.result &&
            typeof message.result === "object" &&
            message.result.outputText === "poll me");
...
//# sourceMappingURL=default-subagent-worker.test.js.map