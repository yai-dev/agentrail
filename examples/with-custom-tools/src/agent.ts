...
import { calculateTool, getCurrentTimeTool } from "./tools.js";
...
export const defaultProfile = defineProfile({
  ...
  agent: {
    ...
    prompt:
      ...
      "get-current-time (returns the current UTC time) and calculate (evaluates arithmetic). " +
      "Use these tools when relevant.",
    tools: [getCurrentTimeTool, calculateTool],
  },
});
