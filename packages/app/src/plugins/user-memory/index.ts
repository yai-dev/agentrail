...
export function createUserMemoryPlugin(service: UserMemoryPluginHooks): AgentrailPlugin {
  return {
    name: "user-memory",
    start: () => service.start(),
    stop: () => service.stop(),
    onRequestStart: ({ tenantId, userId }) => service.beginForegroundActivity(tenantId, userId),
    onRequestEnd: ({ tenantId, userId }) => service.endForegroundActivity(tenantId, userId),
    onTurnPersisted: ({ tenantId, userId }) => service.touchActivity(tenantId, userId),
  };
}
