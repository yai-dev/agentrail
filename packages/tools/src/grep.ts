...
const parametersSchema = Type.Object({
    ...
    type: Type.Optional(
        Type.String({
            description: 'File type to search (e.g. "js", "py", "rust", "ts"). More efficient than glob for standard file types.',
        }),
    ),
...
export const grepTool = tool()
    ...
    .execute(async ({ pattern, path, glob, type, output_mode, multiline, case_insensitive }) => {
        ...
        const cmd = args.map((a) => (a.startsWith("-") ? a : `'${a.replace(/'/g, "'\\''")}'`)).join(" ");
        ...
        try {
            ...
            if (err && typeof err === "object" && "code" in err && (err as NodeJS.ErrnoException).code === "1") {
                return {
