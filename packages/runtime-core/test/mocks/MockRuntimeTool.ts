...
export function createMockTool(
  name: string,
  executeFn?: (toolCallId: string, params: Record<string, unknown>) => Promise<ToolResult>,
): RuntimeTool {
  return {
    name,
    label: name,
 ...
 */
export function createMockToolWithError(name: string, error: Error): RuntimeTool {
  return {
    name,
    label: name,
    description: `Mock tool: ${name}`,
