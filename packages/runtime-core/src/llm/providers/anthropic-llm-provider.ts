...
export class AnthropicLlmProvider implements LlmProvider {
	...
	stream(request: LlmRequest): LlmStream {
		...
		(async () => {
			...
			try {
				...
				stream.push({ type: "error", reason: errorReason, message: this.toAssistantMessage(output) });
				stream.end();
			}
		})();
	...
	private buildParams(request: LlmRequest): Anthropic.Messages.MessageCreateParamsStreaming {
		...
		if (convertedMessages.length >= 2) {
			...
			if (Array.isArray(content) && content.length > 0) {
				(content[content.length - 1] as unknown as Record<string, unknown>).cache_control = { type: "ephemeral" };
				...
				if (typeof msg.content === "string") {
					...
					const blocks = msg.content.map((c): Anthropic.Messages.ContentBlockParam => {
						if (c.type === "text") {
							...
							return {
								type: "image",
								source: { type: "base64", media_type: c.mimeType as "image/jpeg" | "image/png" | "image/gif" | "image/webp", data: c.data },
							};
						}
					});
	...
	private convertToolResultContent(
		...
		return content.map((c): Anthropic.Messages.TextBlockParam | Anthropic.Messages.ImageBlockParam => {
			if (c.type === "text") {
				return { type: "text", text: sanitizeSurrogates(c.text) };
			} else {
				return {
	...
	private convertTools(tools: LlmRequest["tools"]): Anthropic.Messages.Tool[] {
		...
		if (converted.length > 0) {
			(converted[converted.length - 1] as unknown as Record<string, unknown>).cache_control = { type: "ephemeral" };
		}
	...
	private handleContentBlockStart(
		...
		if (block.type === "text") {
			...
			stream.push({ type: "text_start", contentIndex: output.content.length - 1, partial: this.toAssistantMessage(output) });
		} else if (block.type === "thinking") {
			output.content.push({ type: "thinking", thinking: "", thinkingSignature: "", index: event.index });
			stream.push({ type: "thinking_start", contentIndex: output.content.length - 1, partial: this.toAssistantMessage(output) });
			...
			stream.push({ type: "toolcall_start", contentIndex: output.content.length - 1, partial: this.toAssistantMessage(output) });
		}
	}
	...
	private handleContentBlockDelta(
		...
		if (delta.type === "text_delta" && block.type === "text") {
			...
			stream.push({ type: "text_delta", contentIndex: event.index, delta: delta.text || "", partial: this.toAssistantMessage(output) });
		} else if (delta.type === "thinking_delta" && block.type === "thinking") {
			block.thinking = (block.thinking || "") + (delta.thinking || "");
			stream.push({ type: "thinking_delta", contentIndex: event.index, delta: delta.thinking || "", partial: this.toAssistantMessage(output) });
			...
			stream.push({ type: "toolcall_delta", contentIndex: event.index, delta: delta.partial_json || "", partial: this.toAssistantMessage(output) });
		}
	}
	...
	private handleContentBlockStop(
		...
		if (block.type === "text") {
			stream.push({ type: "text_end", contentIndex: event.index, content: block.text || "", partial: this.toAssistantMessage(output) });
		} else if (block.type === "thinking") {
			stream.push({ type: "thinking_end", contentIndex: event.index, content: block.thinking || "", partial: this.toAssistantMessage(output) });
