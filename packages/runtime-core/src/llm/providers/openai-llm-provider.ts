...
export class OpenAiLlmProvider implements LlmProvider {
			...
			try {
				...
				for await (const chunk of openaiStream) {
					if (chunk.usage) {
						...
						const reasoningTokens = (chunk.usage as any).completion_tokens_details?.reasoning_tokens || 0;
					...
					if (choice.delta) {
						if (choice.delta.content && choice.delta.content.length > 0) {
							if (!currentBlock || currentBlock.type !== "text") {
								...
								stream.push({ type: "text_start", contentIndex: contentIndex(), partial: this.toAssistantMessage(output) });
							}
						...
						if (reasoningContent && reasoningContent.length > 0) {
							if (!currentBlock || currentBlock.type !== "thinking") {
								...
								output.content.push(currentBlock);
								stream.push({ type: "thinking_start", contentIndex: contentIndex(), partial: this.toAssistantMessage(output) });
							}
						...
						if (choice.delta.tool_calls) {
							for (const toolCall of choice.delta.tool_calls) {
								if (
									...
									output.content.push(currentBlock);
									stream.push({ type: "toolcall_start", contentIndex: contentIndex(), partial: this.toAssistantMessage(output) });
								}
								...
								if (currentBlock.type === "toolCall") {
									...
									if (toolCall.function?.arguments) {
										...
										currentBlock.partialArgs = (currentBlock.partialArgs || "") + toolCall.function.arguments;
										currentBlock.arguments = parseStreamingJson(currentBlock.partialArgs);
									}
									stream.push({
										type: "toolcall_delta",
					...
					messages.push({
						...
						content: msg.content.map((c) =>
							c.type === "text"
								...
								: { type: "image_url" as const, image_url: { url: `data:${c.mimeType};base64,${c.data}` } },
						),
					});
				}
	...
	private mapStopReason(reason: OpenAI.Chat.ChatCompletionChunk.Choice["finish_reason"]): StopReason {
